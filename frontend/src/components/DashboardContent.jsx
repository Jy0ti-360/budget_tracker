...
const DashboardContent = ({ transactions, summary, refreshData }) => {
  ...
  return (
  <div className="dashboard px-4 py-5 max-w-4xl mx-auto font-sans">
    <h1 className="text-center text-xl sm:text-2xl font-bold text-gray-800 mb-4">
      Transactions Summary of {getMonthYear()}
    </h1>

    <div className="flex flex-wrap justify-center gap-6 my-8">
      <div className="w-[150px] h-[150px] sm:w-[180px] sm:h-[180px] rounded-full flex flex-col justify-center items-center text-white font-bold shadow-md transition-transform duration-300 hover:scale-105 bg-gradient-to-br from-teal-600 to-teal-500">
        <div className="text-lg mb-2 opacity-95">Income</div>
        <div className="text-3xl sm:text-4xl">₹{summary.income.toFixed(2)}</div>
      </div>

      <div className="w-[150px] h-[150px] sm:w-[180px] sm:h-[180px] rounded-full flex flex-col justify-center items-center text-white font-bold shadow-md transition-transform duration-300 hover:scale-105 bg-gradient-to-br from-red-500 to-red-400">
        <div className="text-lg mb-2 opacity-95">Expense</div>
        <div className="text-3xl sm:text-4xl">₹{summary.expense.toFixed(2)}</div>
      </div>
    </div>

    <h1 className="text-2xl font-bold text-gray-800 mb-6">Add a new Transaction</h1>
    <form
      onSubmit={handleNew}
      className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full max-w-[600px] mx-auto mb-8"
    >
      <select
        name="type"
        value={form.type}
        ...
        required
        className="p-2 text-sm border border-gray-300 rounded h-10 w-full appearance-none"
      >
      ...
      <input
        ...
        required
        className="p-2 text-sm border border-gray-300 rounded h-10 w-full"
      />
      ...
      <input
        ...
        className="sm:col-span-2 p-2 text-sm border border-gray-300 rounded h-10 w-full"
      />

      <button
        type="submit"
        className="sm:col-span-2 mx-auto px-6 py-2 bg-[#2876a7] text-white rounded hover:bg-[#215188] transition font-medium text-base"
      >
        Add
      </button>
    </form>
    ...
    <div className="mt-10 space-y-4">
      <h2 className="text-lg font-bold">Upload Transaction File (Excel file)</h2>
      <input
        type="file"
        id="fileInput"
        accept=".xlsx, .xls"
    ...
    {previewMode && (
      <div className="mt-6 border p-4 rounded bg-gray-50 overflow-x-auto">
        <h4 className="font-bold mb-4 text-lg">Preview Extracted Transactions</h4>
        <table className="min-w-[600px] w-full text-sm table-auto border-collapse mb-4">
          <thead>
